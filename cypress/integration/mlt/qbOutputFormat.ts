...
const outputFormatLabels = [
  "Column elements to include: activities", "Column elements to include: transactions"
  ];
  ...
  describe('Query Builder - output format', function() {
    ...
    it('should show the correct text', function() {
      outputFormatLabels.forEach(text => {cy.get(`[label= "${text}"]`).should('exist')});
  ...
  function testSelect(selector: string, selectNumber: number,  optionNumber: number){
    ...
    cy.get(`#react-select-${selectNumber}-option-0-${optionNumber}`).then(($span) => {
      const selectText : string = $span.text()
      cy.get(`#react-select-${selectNumber}-option-0-${optionNumber}`).click();
